...
def hypothesis_test():
    ...
    plt.axvline(observed_stat, color='green', linestyle='--', label=f'Observed {parameter}: {observed_stat:.4f}')
    plt.axvline(hypothesized_value, color='orange', label=f'Hypothesized {parameter} (H₀): {hypothesized_value}')
...
def confidence_interval():
    ...
    plt.scatter(estimates, [0] * len(estimates), alpha=0.5, color='black', label='Simulated Estimates')
    mean_color = 'limegreen' if includes_true else 'darkorange'
    plt.scatter(mean_estimate, 0, color=mean_color, s=200, edgecolor='black', label='Mean Estimate', zorder=5)
    ci_color = 'limegreen' if includes_true else 'darkorange'
    plt.hlines(0, ci_lower, ci_upper, color=ci_color, linewidth=4, label=f'{confidence_level}% Confidence Interval')
    plt.axvline(true_param, color='deepskyblue', linestyle='dotted', linewidth=2, label=f'True {parameter} Value')
    plt.xlabel(f'{parameter.capitalize()} Estimate', fontsize=12)
    plt.title(f'{confidence_level}% Confidence Interval for {parameter.capitalize()} (Mean Estimate)', fontsize=14)
    plt.legend(frameon=True, loc='upper left', framealpha=0.8, facecolor='ghostwhite', edgecolor='black')
    plt.tight_layout()
